...
export interface EmailReminderCard {
  type: string; // e.g. "MOT Expiry", "PCO Licence Expiry", "Rent Due"
  date: string; // e.g. "15 Oct 2026"
  vehicleReg?: string; // e.g. "RE70 VCH"
  expiringInDays?: string | number; // e.g. "7" or 7
  icon?: string; // "mot" | "pco" | "licence" | "rent" | string
}

export interface EmailTemplateReminderData {
  userName?: string;
  badgeLabel?: string; // default "IMPORTANT REMINDER"
  headline: string; // e.g. "Your MOT & PCO expiry dates are approaching."
  subtext?: string; // e.g. "Please submit updated documentation or arrange testing as soon as possible."
  intro?: string; // e.g. "Here is an important update regarding your vehicle compliance and fleet account status:"
  cards?: EmailReminderCard[];
  message?: string; // Single message body for simple reminders without cards
  warningCallout?: string; // Dark warning callout note below cards
  contactInfo?: string; // Line inviting contact for help
}

const REAL_LOGO_URL = "https://virtual-carhire.co.uk/assets/logo.png";

/**
 * Common Footer shared by all templates
 */
...
/**
 * Get icon representation for reminder card types
 */
function getCardIcon(iconType?: string, alertType?: string): string {
  const t = (iconType || alertType || "").toLowerCase();
  if (t.includes("mot")) return "🔺"; // MOT triangle icon symbol
  if (t.includes("pco")) return "🪪"; // PCO badge icon
...
/**
 * Template 2 — Reminder/Alert Notification
 */
