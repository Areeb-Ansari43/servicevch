...
const valueOf = (field: any) =>
  field?.valueString ??
  field?.valueNumber ??
  field?.valueCountryRegion ??
  field?.valueAddress?.streetAddress ??
  field?.content ??
  "";
const readField = (field: any) => String(valueOf(field)).trim();
const readDate = (field: any) =>
  String(field?.valueDate ?? field?.valueString ?? field?.content ?? "").trim();
const normalizeAddress = (value: string) =>
  value
    .replace(/\s+/g, " ")
    .replace(/\s*,\s*/g, ", ")
    .trim();
const findPostcode = (value: string) =>
...
const firstLineMatch = (content: string, pattern: RegExp) =>
  content
    .split(/\r?\n/)
    .map((line) => line.trim())
...
export const scanDrivingLicence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!endpoint || !key)
      throw new Error("Azure Document Intelligence is not configured on the server.");

    const bytes = Uint8Array.from(
      atob(data.imageBase64.replace(/^data:[^;]+;base64,/, "")),
    ...
    try {
      start = await fetch(
        `${endpoint}/documentModels/prebuilt-idDocument:analyze?api-version=2024-11-30`,
        {
          method: "POST",
      ...
      throw new Error(
        error instanceof Error && error.name === "AbortError"
          ? "Azure licence scan timed out after 20 seconds."
          : `Azure licence request failed: ${error instanceof Error ? error.message : "unknown error"}`,
      );
    } finally {
    ...
    if (!result?.analyzeResult?.documents?.[0])
      throw new Error("Azure scan timed out or found no licence document.");

    const fields = result.analyzeResult.documents[0].fields ?? {};
    const ocrContent = String(result.analyzeResult.content ?? "");
    const firstName =
      readField(fieldByName(fields, "FirstName", "GivenName", "FirstNames")) ||
      firstLineMatch(ocrContent, /^1[.)]\s*[A-Z]/i);
    const lastName =
      readField(fieldByName(fields, "LastName", "Surname", "FamilyName")) ||
      firstLineMatch(ocrContent, /^2[.)]\s*[A-Z]/i);
    const fullAddress =
      readField(fieldByName(fields, "Address", "AddressLine", "FullAddress")) ||
      firstLineMatch(ocrContent, /\b(?:street|road|avenue|lane|close|drive|way|court|gardens?)\b/i);
    const postcode = findPostcode(fullAddress || ocrContent);
    const licence =
      readField(fieldByName(fields, "DocumentNumber", "LicenceNumber", "LicenseNumber")) ||
      firstLineMatch(ocrContent, /^5[.)]/i);
    const dob =
      readDate(fieldByName(fields, "DateOfBirth", "BirthDate")) ||
      firstLineMatch(ocrContent, /^3[.)]/i);
    const expiry =
      readDate(fieldByName(fields, "DateOfExpiration", "ExpiryDate", "DateOfExpiry")) ||
      firstLineMatch(ocrContent, /^4[.)]/i);
    return {
