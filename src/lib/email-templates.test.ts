...
describe("Email Templates Foundation", () => {
  ...
  describe("Template 3: Fleet-Wide Expiry Summary (Staff-Facing)", () => {
    it("renders fleet summary pills, sortable vehicle rows, and compliance warning", () => {
      const html = renderFleetSummaryTemplate({
        motCount: 4,
        pcoCount: 6,
        vehicles: [
          {
            registration: "LC71 FGH",
            model: "Toyota Prius",
      ...
      expect(html).toContain("LC71 FGH");
      expect(html).toContain("Toyota Prius");
