...
const HANDOFF_24H = "Our team will get back to you within 24 hours. Please do not contact this number — we will contact you first.";
const AUTO_SURGEON = "The Auto Surgeon, Unit 3 Squirrels Trading Estate, Viveash Close, Hayes UB3 4RZ";
...
function parseJson(text: string): AiIntake | null {
  try {
    const value = JSON.parse(text.replace(/^```json\s*/i, "").replace(/\s*```$/i, "").trim());
    if (typeof value?.reply !== "string" || !value.reply.trim()) return null;
    return {
      reply: value.reply.trim(),
      summary: typeof value.summary === "string" ? value.summary.trim() : value.reply.trim().slice(0, 300),
...
async function classify(kind: IntakeKind, text: string, name: string, issue: string, vehicle: string): Promise<AiIntake> {
  const fallbackReply = kind === "emergency"
  ...
  const fallback: AiIntake = { reply: fallbackReply, summary: `${issue || kind}: ${text}`.slice(0, 300), needs_human: false, reason: "" };
  const apiKey = (getRuntimeEnv("GEMINI_API_KEY") ?? getRuntimeEnv("GOOGLE_API_KEY") ?? getRuntimeEnv("GOOGLE_GEMINI_API_KEY") ?? "").trim();
  if (!apiKey) return { ...fallback, needs_human: true, reason: "gemini_api_key_missing" };

  const system = kind === "emergency"
...
async function handleIntake(request: Request) {
  ...
  const kind: IntakeKind = body.kind === "emergency" ? "emergency" : body.kind === "accident" ? "accident" : "whatsapp";
  ...
  if (!userId) return json({ ok: false, error: "No fleet owner account found", reply: ai.reply }, 500);

  let leadId: string | null = null;
  if (phone) {
    const { data: existing } = await (supabaseAdmin.from("whatsapp_leads") as any).select("id").eq("user_id", userId).eq("phone", phone).order("last_message_at", { ascending: false }).limit(1).maybeSingle();
    leadId = existing?.id ?? null;
  }
  if (!leadId) {
    const { data: created, error } = await (supabaseAdmin.from("whatsapp_leads") as any).insert({
      user_id: userId,
      contact_name: name || "Website visitor",
      phone,
      message: text,
      ai_summary: ai.summary,
      intent: kind === "emergency" ? "emergency_breakdown" : kind === "accident" ? "report_accident" : "support",
      status: ai.needs_human ? "needs_human" : "active",
    }).select("id").single();
    if (error) return json({ ok: false, error: error.message, reply: ai.reply }, 500);
    leadId = created.id;
  } else {
    await (supabaseAdmin.from("whatsapp_leads") as any).update({ contact_name: name || undefined, message: text, ai_summary: ai.summary, status: ai.needs_human ? "needs_human" : "active", last_message_at: new Date().toISOString() }).eq("id", leadId);
  }
  if (leadId) {
    await (supabaseAdmin.from("messages") as any).insert([
      { user_id: userId, lead_id: leadId, sender: "customer", content: text },
      { user_id: userId, lead_id: leadId, sender: "ai_agent", content: ai.reply, handoff: ai.needs_human },
    ]);
  }
  return json({ ok: true, kind, lead_id: leadId, reply: ai.reply, summary: ai.summary, needs_human: ai.needs_human, reason: ai.reason, whatsapp_number: "442072946756" });
}
