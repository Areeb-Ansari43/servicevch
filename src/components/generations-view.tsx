      ...
      <div className={`${panel} overflow-hidden`}>
        ...
        <div className="space-y-6 p-5 sm:p-7">
          {tab === "permission" ? <>
            ...
            <div className="grid gap-5 md:grid-cols-2"><Field label="Start Time" value={contractStartTime} onChange={setContractStartTime} type="time" /><Field label="Return Time" value={contractReturnTime} onChange={setContractReturnTime} type="time" /></div>
          ...
          <div className="flex flex-wrap items-center gap-3"><Field label="Output File Name" value={fileName || (tab === "permission" ? "Permission Letter" : "Contract")} onChange={setFileName} /><button type="button" disabled={generating || scanBusy} onClick={() => void generate()} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#ff6a00] px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-orange-950/30 transition hover:bg-[#ff7a21] active:scale-[.98] disabled:cursor-wait disabled:opacity-60"><Download className="h-4 w-4" /> {generating ? "Generating PDF…" : `Generate ${tab === "permission" ? "Permission Letter" : "2-Page Contract"} PDF`}</button></div>
        </div>
      </div>
    </section>;
}
