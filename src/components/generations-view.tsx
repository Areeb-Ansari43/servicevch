...
export function GenerationsView({ vehicles, drivers, toast }: Props) {
  ...
  const generate = async () => {
    if (!scan.fullName.trim() || !selectedReg.trim()) { toast("Add a driver name and select a vehicle first.", "error"); return; }
    if (!scanReview) { toast("Review and confirm the scanned fields first.", "error"); return; }
    ...
    const filename = `${tab === "permission" ? "Permission-Letter" : "Contract"}-${selectedReg.replace(/\s/g, "")}-${Date.now()}`; const blob = doc.output("blob");
    try { await saveDocument(tab === "permission" ? "permission_letter" : "contract", blob, filename); doc.save(`${filename}.pdf`); toast(`${tab === "permission" ? "Permission letter" : "Contract"} generated and saved.`, "success"); } catch (error) { toast(error instanceof Error ? error.message : "Could not save document.", "error"); }
  };
